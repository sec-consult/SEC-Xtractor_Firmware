...
{
	memset(&params, 0, sizeof(onfi_param_page_t));	
}
...
static void cmdConfigOnfi()
{
	...
	onfi_param_page_full_t param_page;
	nandInit();
	nandReset();
	onfiReadParameters(&param_page);
	if(memcmp(&param_page.param_page[0].signature, "ONFI", 4) == 0){
		if(memcmp(&param_page.param_page[0], &param_page.param_page[1], sizeof(onfi_param_page_t)) != 0){
			uartWriteString(NL"ONFI parameter page copys do not match. Further operation might be prone to errors!"NL);
		}
		memcpy(&params, &param_page.param_page[0], sizeof(onfi_param_page_t));
	}else {
...
static void cmdWriteNand()
{
	if(isOnfiParamConfigured()){
		...
		if(uartGetInput() != 'n'){
			...
			onfiWriteData(&params, 0xbeef, 1);
		}else{
			uartWriteString("Skipping ECC!"NL);
			onfiWriteData(&params, 0xbeef, 0);
		}
		uartWriteString(NL "Operations finished successfully." NL);
	}
}
...
static void cmdDumpNand(char *arguments)
{
	if(isOnfiParamConfigured()){
		...
		while(!uartHasInput());
		if(uartGetInput() != 'n'){
			uartWriteString("Dumping with ECC!"NL);
			onfiReadData(&params, 1);
		}else{
			uartWriteString("Dumping without ECC!"NL);
			onfiReadData(&params, 0);
		}
	}
}
...
*/
static inline void onfiPrintParams(onfi_param_page_t *params)
{
	uartprintf("Manufacturer:\t\t\t%c%c%c%c%c%c%c%c%c%c%c%c"NL,params->vendor[0],params->vendor[1],params->vendor[2],params->vendor[3],params->vendor[4],params->vendor[5],params->vendor[6],params->vendor[7],params->vendor[8],params->vendor[9],params->vendor[10],params->vendor[11]);
	uartprintf("Model:\t\t\t\t%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c"NL,params->model[0],params->model[1],params->model[2],params->model[3],params->model[4],params->model[5],params->model[6],params->model[7],params->model[8],params->model[9],params->model[10],params->model[11],params->model[12],params->model[13],params->model[14],params->model[15],params->model[16],params->model[17],params->model[18],params->model[19]);
